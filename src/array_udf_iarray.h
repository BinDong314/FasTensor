...
public:
  ...
  ~IArrayData(){
    if(iarray_handle >= 0 ){ iArray_delete(iarray_handle); iarray_handle = -1;}
    if(nvs_f_handle != NULL) {delete nvs_f_handle; nvs_f_handle=NULL;}
  }
  
  std::vector<unsigned long long>  GetDimSize(){
    std::vector<unsigned long long> dims_t; dims_t.resize(rank);
  ...
  int ReadData(std::vector<unsigned long long> start, std::vector<unsigned long long> end, std::vector<T> &data){
    ...
    for(int i = 0; i < start.size(); i++) global_count[i] = end[i]-start[i] + 1;
  ...
  int ReadDataVoid(std::vector<unsigned long long> start, std::vector<unsigned long long> end, void *data){
    ...
    std::vector<unsigned long long> global_count; global_count.resize(start.size());
    for(int i = 0; i < start.size(); i++) global_count[i] = end[i]-start[i] + 1;
  ...
  int WriteData(std::vector<unsigned long long> start, std::vector<unsigned long long> end, std::vector<T> data){
    ...
    global_count.resize(rank); global_start.resize(rank);
  ...
  void Nonvolatile(){
    if(iarray_handle < 0){printf("Invalide handle at Nonvolatile !\n"); exit(-1);}
  ...
  void SetHandle(int h){
    ...
    std::vector<long long unsigned int> dims_temp; dims_temp.resize(rank);
