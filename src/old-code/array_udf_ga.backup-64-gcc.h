...
private:
  int  type;
  //#ifdef GACXX
  GA::GlobalArray *g_a =  NULL;
  //#else    
  int  ga_handle = INVALID_HANDLE;
  //#endif
...
public:
  ...
  ~GAData(){
    ...
    //GA_Terminate();
#ifdef GACXX
    if(g_a != NULL){ delete g_a; g_a = NULL;}
#else
    if(ga_handle != INVALID_HANDLE) { GA_Destroy(ga_handle); ga_handle = INVALID_HANDLE; };
#endif
  ...
  int CreateGA(int data_dims, std::vector<unsigned long long> &data_dims_size, AUType data_type_class, std::vector<int> data_overlap_size, std::vector<int> data_chunk_size_t){
    ...
    switch (data_type_class){
...
#else
          ga_handle = NGA_Create(C_INT, data_dims,  &(dims[0]),  name_t, &chunk[0]);
#endif
        }else{
#ifdef GACXX
          char tc;
          g_a =  new  GA::GlobalArray(C_INT, data_dims,  &(dims[0]), &(overlap[0]), name_t, &chunk[0], tc);
          ga_handle = g_a->handle();
#else
#ifdef GA64
          ga_handle = NGA_Create_ghosts64(C_INT, data_dims,  &(dims[0]), &(overlap[0]), name_t, &chunk[0]);
#else
          ga_handle = NGA_Create_ghosts(C_INT, data_dims,  &(dims[0]), &(overlap[0]), name_t, &chunk[0]);
#endif
#endif
      ...
      case AU_FLOAT:
        ...
        if(ghost_flag == 0){
          ...
          #else
          ga_handle = NGA_Create(C_FLOAT, data_dims, &(dims[0]), name_t, &chunk[0]);
          #endif
        }else{
          #ifdef GA64
          ga_handle = NGA_Create_ghosts64(C_FLOAT, data_dims, &(dims[0]), &(overlap[0]), name_t, &chunk[0]);
          #else
          ga_handle = NGA_Create_ghosts(C_FLOAT, data_dims, &(dims[0]), &(overlap[0]), name_t, &chunk[0]);
          #endif
        }
        if (!ga_handle) {printf("Create GA failed !"); exit(-1);};
        break;
    ...
    #else
    std::vector<int> dist_lo, dist_hi;
    dist_lo.resize(rank); dist_hi.resize(rank);
    #ifdef GACXX
      g_a->distribution(ga_me, &(dist_lo[0]), &(dist_hi[0]));
    #else
      NGA_Distribution(ga_handle, ga_me, &(dist_lo[0]), &(dist_hi[0]));
    #endif
      //{printf("Create ga_rank = %d, dist_lo = (%d, %d), dist_hi = (%d, %d)  \n", ga_me, dist_lo[0], dist_lo[1], dist_hi[0], dist_hi[1]); fflush(stdout);}
    #endif
  ...
  int ReadData(std::vector<unsigned long long> start, std::vector<unsigned long long> end, std::vector<T> &data){
    ...
    dist_lo.resize(rank); dist_hi.resize(rank);
      ...
      NGA_Distribution(ga_handle, ga_me, &(dist_lo[0]), &(dist_hi[0]));
    #endif
      //{printf("Read ga_rank = %d, dist_lo = (%d, %d), dist_hi = (%d, %d)  \n", ga_me, dist_lo[0], dist_lo[1], dist_hi[0], dist_hi[1]); fflush(stdout);}
    #endif
    ...
    if(ghost_flag == 0){
      if (rank > 1){
        ...
        #else
        NGA_Get(ga_handle, &(lo[0]),  &(hi[0]), &(data[0]),  &(ld[0]));
        #endif
      }else{
        #ifdef GA64
        NGA_Get64(ga_handle, &(lo[0]),  &(hi[0]), &(data[0]),  NULL);
        #else
        NGA_Get(ga_handle, &(lo[0]),  &(hi[0]), &(data[0]),  NULL);
        #endif
      }
    }else{
...
#else
        #ifdef GACXX
          g_a->getGhostBlock(&(lo[0]),  &(hi[0]), &(data[0]),  &(ld[0]));
        #else
          NGA_Get_ghost_block(ga_handle, &(lo[0]),  &(hi[0]), &(data[0]),  &(ld[0]));
        #endif
#endif
...
#else
        NGA_Get_ghost_block(ga_handle, &(lo[0]),  &(hi[0]), &(data[0]),  NULL);
#endif
        //NGA_Access_ghosts64(ga_handle, &dims_t[0], &data[0], &ld_t[0]);
      }
    ...
    if(mpi_rank < 3  ) { PrintVector("Start: ", start); PrintVector("End: ", end); printf("At rank %d, data vector's size = %d ,,, ", mpi_rank, data.size()); PrintVector(" Rank's read data: ", data);  fflush(stdout);}
  ...
  int WriteData(std::vector<unsigned long long> start, std::vector<unsigned long long> end, std::vector<T> data){
    ...
    dist_lo.resize(rank); dist_hi.resize(rank);
      ...
      NGA_Distribution(ga_handle, ga_me, &(dist_lo[0]), &(dist_hi[0]));
    #endif
      //{printf("Write ga_rank = %d, dist_lo = (%d, %d), dist_hi = (%d, %d)  \n", ga_me, dist_lo[0], dist_lo[1], dist_hi[0], dist_hi[1]); fflush(stdout);}
    #endif
    ...
    for (int i = 0; i < (rank); i++){
      ...
      #else
      lo[i]=(int)start[i]; hi[i]=(int)end[i];
      #endif
    }

    if(mpi_rank < 3  ) { printf("At rank = %d\n", mpi_rank);PrintVector("Start: ", start); PrintVector("End: ", end); PrintVector(" Rank's written data: ", data); fflush(stdout);}
    ...
    if (rank > 1){
      ...
      #else
#ifdef GACXX
      g_a->put(&(lo[0]),  &(hi[0]), &(data[0]),  &(ld[0]));
#else
      NGA_Put(ga_handle,  &(lo[0]),  &(hi[0]), &(data[0]),  &(ld[0]));
#endif
      #endif
    }else{
      #ifdef GA64
      ...
      #else
      NGA_Put(ga_handle,  &(lo[0]),  &(hi[0]), &(data[0]),  NULL);
      #endif
    }
    ...
    if(mpi_rank < 3  ) { printf("At rank = %d\n", mpi_rank);PrintVector("Start: ", start); PrintVector("End: ", end); PrintVector(" Rank's read data after writing: ", data); fflush(stdout);}
  ...
  int WriteDataVoidP(std::vector<unsigned long long> start, std::vector<unsigned long long> end, void *data){
    ...
    if (rank > 1){
      #ifdef GA64
      NGA_Put64(ga_handle,  &(lo[0]),  &(hi[0]), data,  &(ld[0]));
      #else
         #ifdef GACXX
          g_a->put(&(lo[0]),  &(hi[0]), data,  &(ld[0]));
        #else
          NGA_Put(ga_handle,  &(lo[0]),  &(hi[0]), data,  &(ld[0]));
        #endif
      #endif
    }else{
      #ifdef GA64
        NGA_Put64(ga_handle,  &(lo[0]),  &(hi[0]), data,  NULL);
      #else
        NGA_Put(ga_handle,  &(lo[0]),  &(hi[0]), data,  NULL);
      #endif
    }
  ...
  void Nonvolatile(){
    ...
    #else
      #ifdef GACXX
      g_a->distribution(ga_me, &(lo[0]), &(hi[0]));
      #else
      NGA_Distribution(ga_handle, ga_me, &(lo[0]), &(hi[0]));
      #endif
      //NGA_Distribution(ga_handle, ga_me, &(lo[0]), &(hi[0]));
    for (int i = 0; i < (rank-1); i++){
      ld[i]=(int)(hi[i]-lo[i]+1);
    }
    #endif
