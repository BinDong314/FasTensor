...
public:
  ...
  ~GAData(){
    ...
    //GA_Terminate();
    if (ga_handle) GA_Destroy(ga_handle);
  ...
  int CreateGA(int data_dims, std::vector<unsigned long long> &data_dims_size, AUType data_type_class, std::vector<int> data_overlap_size){
    #ifdef DEBUG
    if(mpi_rank == 0){printf("Debug: create GA, in udf_ga. ghost size = (%d,) \n", data_overlap_size[0]); fflush(stdout);}
    #endif
    rank = data_dims;
    type = data_type_class;
    dims.resize(rank); lo.resize(rank); hi.resize(rank); ld.resize(rank-1); overlap.resize(rank);
    ...
    switch (data_type_class){
      case AU_INTEGER:
        if(ghost_flag == 0){
          ga_handle = NGA_Create64(C_INT, data_dims,  &(dims[0]),  name_t, NULL);
        }else{
          ga_handle = NGA_Create_ghosts64(C_INT, data_dims,  &(dims[0]), &(overlap[0]), name_t, NULL);
          if(mpi_rank == 0) {printf("Create GA array with ghost zone, voerlap size = (%d, %d),  ga_hanlde=%d \n", overlap[0], overlap[1], ga_handle); fflush(stdout);}
        }
      ...
      case AU_FLOAT:
        ...
        if(ghost_flag == 0){
          ga_handle = NGA_Create64(C_FLOAT, data_dims, &(dims[0]), name_t, NULL);
        }else{
          ga_handle = NGA_Create_ghosts64(C_FLOAT, data_dims, &(dims[0]), &(overlap[0]), name_t, NULL);
        }
        if (!ga_handle) {printf("Create GA failed !"); exit(-1);};
        break;
      default:
  ...
  int ReadData(std::vector<unsigned long long> start, std::vector<unsigned long long> end, std::vector<T> &data){
    ...
    dist_lo.resize(2); dist_hi.resize(2);
