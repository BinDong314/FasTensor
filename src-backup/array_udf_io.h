...
private:
  ...
  GAData<T>      *vs_f_handle =  NULL; // Can be void * to support more IO drivers
...
public: 
  ...
  Data(std::string fn_p, std::string gn_p, std::string dn_p, DataOrigin d_orig_p, NVSFile nvs_f_p){
    fn=fn_p; gn=gn_p;dn=dn_p;d_orig=d_orig_p;nvs_f=nvs_f_p;
  ...
  int CreateStorageSpace(int data_dims, std::vector<unsigned long long> &data_dims_size, int data_type_class, std::vector<int> data_overlap_size){
    //printf("Debug: create ss, in udf_io.\n");
    data_dims_t = data_dims; data_dims_size_t =  data_dims_size;data_type_class_t = data_type_class;
    if(cache_flag == AU_NOCACHE){
      ...
      //Create NVS file in case when we need to flush data onto disk
      //vs_f_handle->CreateNVSFile(data_dims, data_dims_size, data_type_class);
      return   vs_f_handle->CreateGA(data_dims, data_dims_size, t, data_overlap_size);
    }
  }
