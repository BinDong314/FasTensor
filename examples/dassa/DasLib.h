...
inline bool CausalityFlagging(std::vector<std::vector<T>> &v, double tmin, double tmax, double fmax, double start_t, double end_t, double smaple_rate)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        filtfilt(B, A, tv1[i], tv2[i]);
        tv3[i] = TimeSubset(tv2[i], start_t, end_t, tmin, tmax * 0.9, smaple_rate);   //causal
        tv4[i] = TimeSubset(tv2[i], start_t, end_t, -tmax * 0.9, -tmin, smaple_rate); //acausal
    }
