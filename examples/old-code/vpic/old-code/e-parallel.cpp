...
inline float Eparallel_UDF(const Stencil<HydroPoint> &hp) 
{
  HydroPoint hpt = hp(0,0,0);
  return (hpt.ex * hpt.ex + hpt.ey * hpt.ey + hpt.ez * hpt.ez)/sqrt(hpt.bx * hpt.bx + hpt.by * hpt.by + hpt.bz * hpt.bz);
}
...
int main(int argc, char *argv[])
{
  ...
  char i_file[1024]="/Users/dbin/work/vpic-hdf5/vpic/build/testdir/field_hdf5/T.50/fields_50.h5";
  char o_file[1024]="/Users/dbin/work/vpic-hdf5/vpic/build/testdir/field_hdf5/T.50/fields_50_Eparallel.h5";
  
  char  group[1024] ="/Timestep_50";
  char  dsetEX[1024]="/Timestep_50/ex";
  char  dsetEY[1024]="/Timestep_50/ey";
  char  dsetEZ[1024]="/Timestep_50/ez";
  char  dsetBX[1024]="/Timestep_50/cbx";
  char  dsetBY[1024]="/Timestep_50/cby";
  char  dsetBZ[1024]="/Timestep_50/cbz";
  char  dsetM[1024] ="/Timestep_50/eparallel";
  ...
  //Orginal data sets
  Array<float> *EX = new Array<float>(AU_NVS,  AU_HDF5, i_file, group,  dsetEX, chunk_size, overlap_size);
  Array<float> *EY = new Array<float>(AU_NVS,  AU_HDF5, i_file, group,  dsetEY, chunk_size, overlap_size);
  Array<float> *EZ = new Array<float>(AU_NVS,  AU_HDF5, i_file, group,  dsetEZ, chunk_size, overlap_size);
  Array<float> *BX = new Array<float>(AU_NVS,  AU_HDF5, i_file, group,  dsetBX, chunk_size, overlap_size);
  Array<float> *BY = new Array<float>(AU_NVS,  AU_HDF5, i_file, group,  dsetBY, chunk_size, overlap_size);
  Array<float> *BZ = new Array<float>(AU_NVS,  AU_HDF5, i_file, group,  dsetBZ, chunk_size, overlap_size);

  
  //Create a compound data strucutre to include JX/JY/JZ
  Array<HydroPoint, float> *XYZ = new Array<HydroPoint, float>(AU_VIRTUAL);
  XYZ->SetAttributes(EX, EY, EZ, BX, BY, BZ); 
