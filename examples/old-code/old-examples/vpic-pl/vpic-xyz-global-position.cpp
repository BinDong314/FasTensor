...
//Use some variables to help the performance
...
inline float Domain_UDF(const Stencil<float> &p){
  ...
  int   domain_index;
  unsigned long long np_local_acc = prev_np_local_acc;  
  int np_local_acc_temp;
...
inline float Z_UDF(const Stencil<Particle> &p) 
{
  ...
  z_v = ( (int)pt.i / ((nx_v + 2) * (ny_v + 2)) + (pt.dZ - 1)/2.0 ) * dz_v + z0_v;
  return  z_v;
}
...
int main(int argc, char *argv[])
{
  ...
  // 23488064664
  char i_file[1024]="/global/cscratch1/sd/dbin/vpic-fanguo/T.10400/electron_tracer.h5p";
  char t_file[1024]="/global/cscratch1/sd/dbin/vpic-fanguo/T.10400/grid_metadata_electron_tracer.h5p";
  char o_file[1024]="/global/cscratch1/sd/dbin/vpic-fanguo/T.10400/electron_tracer_output.h5p";
  char o_file_xyz[1024]="/global/cscratch1/sd/dbin/vpic-fanguo/T.10400/electron_tracer_output_xyz.h5p";

  
  char  group[1024]="/Step#10400";
  char dataset[1024]="/Step#10400/dX";

  std::vector<int> chunk_size(1) ;
  std::vector<int> overlap_size(1);
  chunk_size[0]   = 1000000; 
  overlap_size[0] = 0; 
  ...
  if(convert_flag == 1){
    ...
    Array<float> *i1 = new Array<float>(AU_COMPUTED,  AU_HDF5, i_file, group,   "if", chunk_size, overlap_size);
    i0->Apply(I_UDF, i1);
    delete i0;
    delete i1;
    ...
    Array<float> *ii2 = new Array<float>(AU_COMPUTED,  AU_HDF5, i_file, group,   "domain_id", chunk_size, overlap_size);
  ...
  nx       = new Array<int>(AU_NVS, AU_HDF5, t_file, group, "nx", AU_PRELOAD);
  ...
  ny       = new Array<int>(AU_NVS, AU_HDF5, t_file, group, "ny", AU_PRELOAD);
  ...
  nz       = new Array<int>(AU_NVS, AU_HDF5, t_file, group, "nz", AU_PRELOAD);
  dz       = new Array<float>(AU_NVS, AU_HDF5, t_file, group, "dz", AU_PRELOAD);
  

  
  //Results data sets
  Array<float> *X = new Array<float>(AU_COMPUTED, AU_HDF5, o_file_xyz, group, "x", chunk_size, overlap_size);
  //Results data sets
  P->Apply(X_UDF, X);  
  delete X;

  Array<float> *Y = new Array<float>(AU_COMPUTED, AU_HDF5, o_file_xyz, group, "y", chunk_size, overlap_size);
  P->Apply(Y_UDF, Y);  
  delete Y;

  //Results data sets
  Array<float> *Z = new Array<float>(AU_COMPUTED, AU_HDF5, o_file_xyz, group, "z", chunk_size, overlap_size);
  P->Apply(Z_UDF, Z);  
  delete Z;
