...
int main(int argc, char *argv[])
{
  ...
  std::vector<int> mirrors; mirrors.push_back(f_value);
  ...
  if(cache_flag == 1) B->EnableCache();
  if(mirrors_flag == 1) B->SetMirrorValue(mirrors);
  if(replace_flag == 1) B->EnableApplyReplace();
  if(writeback_flag == 1) B->EnableApplyWriteback();
  ...
  t2 = MPI_Wtime(); 
  if(mpi_rank == 0) {printf( "Elapsed time for assigning labels:  %f\n", t2 - t1 );fflush(stdout);}
  A->ReportTime();
  delete A;
  ...
  t2 = MPI_Wtime();
  if(mpi_rank == 0) {printf( "Coverge after %d  iterations. Elapsed time for nonvolatile is %f\n", ite, t2 - t1 );  fflush(stdout);}
  
  B->Nonvolatile();
  t3 = MPI_Wtime(); 
  if(mpi_rank == 0) {printf( "Elapsed time for nonvolatile is %f\n", t3 - t2 );fflush(stdout);}
