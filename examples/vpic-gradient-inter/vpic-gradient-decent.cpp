...
const float bshape[3]      = {512,   512,  256};
const float borigin[3]     = {0,    -165,  -66};
const float originShift[3] = {0 ,    165,   66};
const float bsize[3]       = {330,   330,  132};
const float bcellSize[3]   = {0.64453125, 0.64453125, 0.2578125}; //bsize/(bshape-1.0)
...
int main(int argc, char *argv[])
{
    ...
    // 123, 463, 188960
  char i_file_bx[1024]=      "/global/cscratch1/sd/dbin/de-test-all-osts/vpic-sc18/bx.h5p";
  char i_file_particle[1024]="/global/cscratch1/sd/dbin/de-test-all-osts/vpic-sc18/eparticle.h5p";
  char o_file_bx[1024]=      "/global/cscratch1/sd/dbin/de-test-all-osts/vpic-sc18/bx-new.h5p";
  char o_file_particle[1024]="/global/cscratch1/sd/dbin/de-test-all-osts/vpic-sc18/eparticle-new.h5p";

 char  group[1024]="/Step#2";
  char dataset[1024]="/Step#2/bx";
  ...
  M = new Array<float>(AU_NVS,  AU_HDF5, i_file_bx, "/Step#4", "/Step#4/bx", AU_PRELOAD);
  //rank, dims, chunk_size, overlap_size
  //std::cout <<  "Before Gradient at M(1,1,1): " << M->operator()(1, 1, 1) << std::endl;
  M->EnableApplyWriteback(); 
  M->Apply(GradientUDF, NULL);
  ...
  //Results data sets
  Array<float> *PN = new Array<float>(AU_COMPUTED, AU_HDF5, o_file_particle, group, "mg", chunk_size, overlap_size);
  //PN->EnableCache();
